// insert at end in singly circular linked list
...
// struct node *insertAtEndSCLL(struct node *head, int data)
// {
...
// }
// no need to return so i use void 

void insertAtEndSCLL(struct node *head, int data)
{
    struct node *newnode = createNode(data);
    struct node *current = head;
...
int main() {
  ...
  insertAtEndSCLL(head, 99);

  printSCLL(head);

  insertAtEndSCLL(head, 999);
